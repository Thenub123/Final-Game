...
public class Laser : MonoBehaviour
{
    ...
    void Update() {
        ...
        RaycastHit2D rayGround = Physics2D.Raycast(transform.position, prevPos, 200, groundLayer);

        if(enemyEnabled) {

            canSee = Physics2D.OverlapBox(ray.point, new Vector2(0.1f, 0.1f), 0, playerLayer);
            if(canSee) {
